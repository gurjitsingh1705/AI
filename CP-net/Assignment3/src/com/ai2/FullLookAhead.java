...
public class FullLookAhead {
	...
	public FullLookAhead(String[] variables, Map<ArrayList<String>, Set<ArrayList<Integer>>> constraints, ArrayList<Integer> domain
			, Map<String, Set<String>> dependencies ,Map<String, Map<String, ArrayList<Integer>>> cpt, Map<Integer, ArrayList<Node>> solutions) {
	...
	public ArrayList<Node> fullLookAhead(int level) {
		...
		else if(level>=variables.length) {
				...
				if(node.assignedVals.equals(node.unassignedVals)) {
					...
					{////////////////////////////loop for fulllookahead
						if (variables.length==level || ((variables.length-1)==level))
						{
							...
							for(int i=0;i<node.getUnassignedVals().size();i++) {
								if(fullConsistency(node.getUnassignedVals().get(i), node, level) == true ){
									node.value  =getUnassignedValsFromNode(node).get(0);
									node.assignedVals.add(node.value);
									node.setUnassignedVals(getUnassignedValsFromNode(node));
									status = true;
	...
	private boolean fullConsistency(Integer value, Node node, int level) {
		// TODO Auto-generated method stub
		boolean status = false;
		...
		if(level<variables.length-1) {
			for(int k=0;k<variables.length;k++) {
				if((k==level) && level == variables.length) {
					break;
				}
				var2 = variables[k];
				compatibility = fullConsmethod( value,  level , var1,  var2);
				if(!compatibility) {
					break;
				}
			}
	...
	public boolean fullConsmethod(Integer value, int level ,String var1, String var2)
	{	
		boolean status = false;
